...
class SmartMoneyAnalyzer:
    ...
    def detect_leg(self, df: pd.DataFrame, size: int = None) -> Leg:
        ...
        return (
            Leg.BEARISH
            if new_leg_high.iloc[-1]
            else Leg.BULLISH
            if new_leg_low.iloc[-1]
            else self.current_leg
        )
    ...
    def get_parsed_price(self, df: pd.DataFrame, index: int) -> Tuple[float, float]:
        ...
        if is_volatile:
            return df["low"].iloc[index], df["high"].iloc[
                index
            ]  # Reversed for volatile bars
    ...
    def identify_order_blocks(self, df: pd.DataFrame) -> List[OrderBlock]:
        ...
        for i in range(3, len(df)):
            ...
            if (
                df["close"].iloc[i] > df["open"].iloc[i]  # Current candle bullish
                and df["close"].iloc[i - 1]
                < df["open"].iloc[i - 1]  # Previous candle bearish
                and df["volume"].iloc[i] > volume_sma.iloc[i] * self.volume_threshold
            ):
                order_blocks.append(
                    OrderBlock(
                        index=i,
            ...
            # Check for bearish order block
                ...
                df["close"].iloc[i] < df["open"].iloc[i]  # Current candle bearish
                and df["close"].iloc[i - 1]
                > df["open"].iloc[i - 1]  # Previous candle bullish
                and df["volume"].iloc[i] > volume_sma.iloc[i] * self.volume_threshold
            ):
                order_blocks.append(
                    OrderBlock(
    ...
    def detect_structure_points(
        self, df: pd.DataFrame, internal: bool = False
    ) -> List[StructurePoint]:
        """
        ...
        for i in range(size + 1, len(df)):
            ...
            if internal and self.structure_confluence_filter:
                bullish_bar = df["high"].iloc[i] - max(
                    df["close"].iloc[i], df["open"].iloc[i]
                ) > min(df["close"].iloc[i], df["open"].iloc[i] - df["low"].iloc[i])
                bearish_bar = df["high"].iloc[i] - max(
                    df["close"].iloc[i], df["open"].iloc[i]
                ) < min(df["close"].iloc[i], df["open"].iloc[i] - df["low"].iloc[i])
            ...
            if df["close"].iloc[i] > df["high"].iloc[i - 1]:
                structure_type = (
                    StructureType.CHOCH if trend == Bias.BEARISH else StructureType.BOS
                )
            ...
            # Bearish structure break
                ...
                structure_type = (
                    StructureType.CHOCH if trend == Bias.BULLISH else StructureType.BOS
                )
                structure_points.append(
                    StructurePoint(
                        index=i,
    ...
    def detect_swing_points(self, df: pd.DataFrame) -> List[SwingPoint]:
        ...
        for i in range(self.swing_length + 1, len(df)):
            ...
            if (
                ...
                swing_points.append(
                    SwingPoint(
                        ...
                        creation_time=df.index[i],
                        strength="strong"
                        if self.current_trend == Bias.BULLISH
                        else "weak",
                    )
                )
            ...
            # Lower Low
                ...
                df["low"].iloc[i] < df["low"].iloc[i - 1] and current_leg == Leg.BEARISH
            ):
                swing_points.append(
                    SwingPoint(
                        index=i,
                        ...
                        creation_time=df.index[i],
                        strength="strong"
                        if self.current_trend == Bias.BEARISH
                        else "weak",
                    )
                )
            ...
            # Lower High
                ...
                df["high"].iloc[i] < self.last_swing_high and current_leg == Leg.BEARISH
            ):
                swing_points.append(
                    SwingPoint(
                        index=i,
