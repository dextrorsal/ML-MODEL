...
from ...features.base_torch_indicator import BaseTorchIndicator, TorchIndicatorConfig
...
class LorentzianClassifier(nn.Module):
    ...
    def __init__(
        ...
        # Attention mechanism
        self.attention = nn.Sequential(nn.Linear(hidden_size, 1), nn.Sigmoid())
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        positive_distances = self.lorentzian(
            attended_features,
            self.positive_prototype.expand(attended_features.size(0), -1),
        )
        negative_distances = self.lorentzian(
            attended_features,
    ...
    def forward_with_features(self, features: MLFeatures) -> Dict[str, torch.Tensor]:
        ...
        combined_features = torch.cat(
            [
                ...
                features.volume.view(feature_length, -1)
                if features.volume is not None
                else torch.zeros(feature_length, 1),
            ],
            dim=1,
        )
