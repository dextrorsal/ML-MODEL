...
from ...indicators.smart_money_concepts import SmartMoneyAnalyzer
from .geometric_patterns import GeometricPatternDetector, PatternType
from ..technical.indicators.base_torch_indicator import (
    BaseTorchIndicator,
...
class PatternFeatureExtractor(BaseTorchIndicator):
    def __init__(
        ...
        self.geometric_detectors = (
            {
                tf: GeometricPatternDetector(torch_config=torch_config)
    ...
    def extract_features(self, df: pd.DataFrame, timeframe: str) -> Dict:
        ...
        if self.use_smc and timeframe in self.smc_analyzers:
            ...
            active_obs = torch.tensor(
                [ob.is_valid for ob in order_blocks], device=self.device
            )
            valid_fvgs = torch.tensor(
                [fvg.is_valid for fvg in fair_value_gaps], device=self.device
            )
            bos_points = torch.tensor(
                [sp.type == "BOS" for sp in structure_points], device=self.device
            )
            choch_points = torch.tensor(
                [sp.type == "CHoCH" for sp in structure_points], device=self.device
            )
            ...
            features.update(
                self.calculate_ml_features(ohlcv_tensors, order_blocks, fair_value_gaps)
            )
        ...
        if self.use_geometric and timeframe in self.geometric_detectors:
            ...
            pattern_tensors = {
                pattern_type: torch.tensor(
                    [1 if p.pattern_type == pattern_type else 0 for p in patterns],
                    device=self.device,
                )
            ...
            if patterns:
                confidences = torch.tensor(
                    [p.confidence for p in patterns], device=self.device
                )
                volume_confirms = torch.tensor(
                    [p.volume_confirmation for p in patterns], device=self.device
                )

                features.update(
                    {
                        f"{timeframe}_avg_pattern_confidence": float(
                            torch.mean(confidences).item()
                        ),
                        f"{timeframe}_volume_confirmed_patterns": int(
                            torch.sum(volume_confirms).item()
                        ),
                    }
                )
                ...
                features.update(
                    self.calculate_pattern_ml_features(patterns, ohlcv_tensors)
                )

        return features

    def calculate_ml_features(
        self, ohlcv: Dict[str, torch.Tensor], order_blocks: List, fair_value_gaps: List
    ) -> Dict[str, float]:
        ...
        with torch.no_grad():
            ...
            if order_blocks:
                ob_sizes = torch.tensor(
                    [ob.size for ob in order_blocks], device=self.device
                )
                ob_ages = torch.tensor(
                    [ob.age for ob in order_blocks], device=self.device
                )
                ...
                features.update(
                    {
                        ...
                        "active_ob_ratio": len(
                            [ob for ob in order_blocks if ob.is_valid]
                        )
                        / len(order_blocks),
                    }
                )
            ...
            if fair_value_gaps:
                fvg_sizes = torch.tensor(
                    [fvg.size for fvg in fair_value_gaps], device=self.device
                )
                fvg_ages = torch.tensor(
                    [fvg.age for fvg in fair_value_gaps], device=self.device
                )
                ...
                features.update(
                    {
                        ...
                        "active_fvg_ratio": len(
                            [fvg for fvg in fair_value_gaps if fvg.is_valid]
                        )
                        / len(fair_value_gaps),
                    }
                )
    ...
    def calculate_pattern_ml_features(
        self, patterns: List, ohlcv: Dict[str, torch.Tensor]
    ) -> Dict[str, float]:
        ...
        with torch.no_grad():
            if patterns:
                ...
                durations = torch.tensor(
                    [p.duration for p in patterns], device=self.device
                )
                confidences = torch.tensor(
                    [p.confidence for p in patterns], device=self.device
                )
                ...
                completed = torch.tensor(
                    [p.is_completed for p in patterns], device=self.device
                )
    ...
    def get_training_features(
        self, data: Dict[str, pd.DataFrame], window_size: int = 100
