...
import pandas as pd
import numpy as np
from src.models.strategy.risk_management.chandelier_exit import ChandelierExit, ChandelierConfig
...
def test_chandelier_exit_risk_levels(sample_data, device):
    ...
    assert all(stops_distance[i] < stops_distance[i+1] 
...
def test_chandelier_exit_integration(sample_data, device):
    ...
    # Verify continuity
    assert len(initial_signals['long_stop']) == len(initial_data), \
