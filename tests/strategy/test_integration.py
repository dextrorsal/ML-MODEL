...
class StrategyTester:
        ...
        for i in range(len(data)):
            ...
            if len(self.positions) < self.max_positions:
                entry = self._check_entry(
                    data.iloc[i], primary_signals, confirmed_signals, i
                )
                if entry:
    ...
    def _check_entry(self, current_bar, primary_signals, confirmed_signals, idx):
        ...
        long_signal = (
            primary_signals["buy_signals"][idx] == 1
            and confirmed_signals["predictions"][idx] > self.min_confidence
        )

        # Short entry conditions
        short_signal = primary_signals["sell_signals"][idx] == 1 and confirmed_signals[
            "predictions"
        ][idx] < (1 - self.min_confidence)
...
class TestStrategy:
    def test_strategy_backtest(self, sample_data):
        ...
        strategy = StrategyTester(
            initial_capital=10000, max_positions=2, min_confidence=0.3
        )
        ...
        assert (
            len(results["equity_curve"]) == len(sample_data) + 1
        )  # +1 for initial capital
