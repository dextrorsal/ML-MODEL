...
class TestStrategyComponents:
    def test_lorentzian_classifier(self, sample_data):
        ...
        # Test signal bounds
        assert torch.all(
            (signals["long_signals"] >= 0) & (signals["long_signals"] <= 1)
        )
        assert torch.all(
            (signals["short_signals"] >= 0) & (signals["short_signals"] <= 1)
        )
    ...
    def test_logistic_regression(self, sample_data):
        ...
        assert torch.all(
            (signals["predictions"][valid_mask] >= 0)
    ...
    def test_chandelier_exit(self, sample_data):
        ...
        close_tensor = torch.tensor(
            ...
            dtype=torch.float32,
            device=signals["long_stop"].device,
        )
        ...
        long_stop_below = (
            (signals["long_stop"][valid_mask] <= close_tensor[valid_mask])
            .float()
            .mean()
        )
        short_stop_above = (
            (signals["short_stop"][valid_mask] >= close_tensor[valid_mask])
            .float()
            .mean()
        )
        ...
        assert (
            long_stop_below >= 0.8
        ), f"Only {long_stop_below:.2f} of long stops are below close prices"
        assert (
            short_stop_above >= 0.8
        ), f"Only {short_stop_above:.2f} of short stops are above close prices"
    ...
    def test_strategy_integration(self, sample_data):
        ...
        long_entries = (
            ...
            * (confirmation["predictions"] > 0.5).float()
        )
        short_entries = (
