...
def test_indicator_calculation(sample_price_data):
    ...
    wt1 = wt_signals['wt1'].cpu().numpy() if isinstance(wt_signals['wt1'], torch.Tensor) else wt_signals['wt1'].values
    rsi_vals = rsi_signals['rsi'].cpu().numpy() if isinstance(rsi_signals['rsi'], torch.Tensor) else rsi_signals['rsi'].values
    
    # Check for non-NaN values after warmup
    assert not np.isnan(wt1[warmup:]).all(), "WaveTrend signals are all NaN after warmup"
...
def test_timeframe_resampling(sample_price_data):
    ...
    # Verify dimensions
    assert len(df_4h) == len(df_1h) // 4 + (1 if len(df_1h) % 4 > 0 else 0), "Incorrect number of rows after resampling"
    ...
    for i in range(min(5, len(df_4h))):  # Check first 5 candles
        ...
        assert abs(component_high - resampled_high) < 0.0001, f"High value mismatch for candle {i}"
        ...
        assert abs(component_volume - resampled_volume) < 0.0001, f"Volume mismatch for candle {i}"
