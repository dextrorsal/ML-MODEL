...
NEON_TEST_DB = os.environ.get(
    "NEON_TEST_DB", "postgresql://postgres:test@localhost:5432/solana_trading_test"
)
...
async def test_data_indicator_compatibility(sample_data):
    ...
    # Check WaveTrend
    assert (
        wt_signals["wt1"][warmup_period:].isnan().sum() < len(df) * 0.05
    ), "Too many NaN values in WaveTrend"

    # Check RSI
    assert (
        rsi_signals["rsi"][warmup_period:].isnan().sum() < len(df) * 0.05
    ), "Too many NaN values in RSI"

    # Check ADX
    assert (
        adx_signals["ADX"][warmup_period:].isnan().sum() < len(df) * 0.05
    ), "Too many NaN values in ADX"

    # Check CCI
    assert (
        cci_signals["cci"][warmup_period:].isnan().sum() < len(df) * 0.05
    ), "Too many NaN values in CCI"
...
async def test_data_lorentzian_compatibility(sample_data):
    ...
    try:
        ...
        assert len(signals["predictions"]) == len(
            df
        ), "Classifier predictions length mismatch"
        assert len(signals["buy_signals"]) == len(
            df
        ), "Classifier buy signals length mismatch"
        assert len(signals["sell_signals"]) == len(
            df
        ), "Classifier sell signals length mismatch"
        ...
        valid_signals = (
            signals["buy_signals"][warmup_period:]
            + signals["sell_signals"][warmup_period:]
        ).sum()
...
async def test_timeframe_transformation(sample_data):
    ...
    start_time = max(
        df_15m_resampled["timestamp"].min(), df_15m_actual["timestamp"].min()
    )
    end_time = min(
        df_15m_resampled["timestamp"].max(), df_15m_actual["timestamp"].max()
    )

    df_15m_resampled = df_15m_resampled[
        (df_15m_resampled["timestamp"] >= start_time)
        & (df_15m_resampled["timestamp"] <= end_time)
    ]

    df_15m_actual = df_15m_actual[
        (df_15m_actual["timestamp"] >= start_time)
        & (df_15m_actual["timestamp"] <= end_time)
    ]

    # Allow for small differences due to slight differences in candle boundaries
    close_diff = (
        abs(df_15m_resampled["close"].mean() - df_15m_actual["close"].mean())
        / df_15m_actual["close"].mean()
    )
    volume_diff = (
        abs(df_15m_resampled["volume"].sum() - df_15m_actual["volume"].sum())
        / df_15m_actual["volume"].sum()
    )

    assert (
        close_diff < 0.05
    ), "Large discrepancy in closing prices between resampled and actual data"
    assert (
        volume_diff < 0.10
    ), "Large discrepancy in volumes between resampled and actual data"
...
async def test_data_streaming_to_indicators(collector):
                    ...
                    processed_data.append(
                        {
                            ...
                            "close": float(k["c"]),
                            "wt1": wt_signals["wt1"].item()
                            if not wt_signals["wt1"].isnan()[0]
                            else None,
                            "rsi": rsi_signals["rsi"].item()
                            if not rsi_signals["rsi"].isnan()[0]
                            else None,
                        }
                    )
    ...
    # Override the on_message method to use our indicator processor
    def custom_on_message(self, ws, message):
        # Process with original handler for normal operation
        original_on_message(self, ws, message)
        # Also process with our custom handler
        indicator_processor(message)

    # Patch the collector with our custom message handler
    collector.on_message = lambda ws, message: custom_on_message(collector, ws, message)
    ...
    try:
        ...
        if len(processed_data) > 0:
            ...
            for i, data in enumerate(processed_data):
                logger.info(
                    f"Candle {i+1}: Close={data['close']}, WT1={data['wt1']}, RSI={data['rsi']}"
                )
        ...
        collector.on_message = original_on_message
