...
from src.features.chandelier_exit import ChandelierExitIndicator
...
class IndicatorValidator:
    ...
    def validate_sensitivity(self, indicator_name: str) -> Tuple[float, float]:
        ...
        for noise in noise_levels:
            ...
            noisy_data["close"] = noisy_data["close"] * (
                1 + np.random.normal(0, noise, len(self.data))
            )
        ...
        correlations = [
            np.corrcoef(b, n)[0, 1] for b, n in zip(base_signals, noisy_signals)
        ]
        sensitivity = 1 - np.mean(correlations)

        self.results[indicator_name].update(
            {"sensitivity_score": sensitivity, "noise_correlations": correlations}
        )
    ...
    def validate_signal_distribution(self, indicator_name: str) -> Dict:
        ...
        self.results[indicator_name].update({"signal_distribution": signal_stats})
